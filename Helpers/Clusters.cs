...
namespace ScourgeBloom.Helpers
{
    internal static class Clusters
    {
        ...
        public static IEnumerable<WoWUnit> GetCluster(WoWUnit target, IEnumerable<WoWUnit> otherUnits, ClusterType type,
            float clusterRange)
        {
            if (target == null)
            ...
            switch (type)
            {
                case ClusterType.Radius:
                    return (from u in units
                        select new {Count = GetRadiusClusterCount(u, units, clusterRange), Unit = u}).OrderByDescending(
                            a => a.Count).
                        FirstOrDefault().Unit;
                case ClusterType.Chained:
                    return (from u in units
                        select new {Count = GetChainedClusterCount(u, units, clusterRange), Unit = u}).OrderByDescending
                        (a => a.Count).
                        FirstOrDefault().Unit;
                case ClusterType.PathToUnit:
                    return (from u in units
                        select new {Count = GetPathClusterCount(u, units, clusterRange), Unit = u}).OrderByDescending(
                            a => a.Count).
                        FirstOrDefault().Unit;
        ...
        public static IEnumerable<WoWUnit> GetConeCluster(float coneDegrees, float coneDist,
            IEnumerable<WoWUnit> otherUnits)
        {
            return otherUnits
        ...
        private static IEnumerable<WoWUnit> GetRadiusCluster(WoWUnit target, IEnumerable<WoWUnit> otherUnits,
            float radius)
        {
        ...
        public static int GetChainedClusterCount(WoWUnit target, IEnumerable<WoWUnit> otherUnits, float chainRange,
            SimpleBooleanDelegate avoid = null)
        {
            if (avoid == null)
        ...
        private static IEnumerable<WoWUnit> GetChainedCluster(WoWUnit target, IEnumerable<WoWUnit> otherUnits,
            float chainRange)
        {
        ...
        private static IEnumerable<WoWUnit> GetPathToUnitCluster(WoWUnit target, IEnumerable<WoWUnit> otherUnits,
            float distance)
        {
            ...
            float distance)
        {
            ...
            return
                otherUnits.Where(
                    u =>
                        distance + u.CombatReach <=
                        u.Location.GetNearestPointOnSegment(myLoc, destLoc).Distance(u.Location));
        }
        ...
        private static WoWUnit GetChainTarget(WoWUnit from, IEnumerable<WoWUnit> otherUnits,
