             ...
             if (StyxWoW.Me.GotAlivePet && StyxWoW.Me.Pet != null)
             {
                 if (_petGuid != StyxWoW.Me.Pet.Guid)
                 {
                     ...
                     if (StyxWoW.Me.PetSpells.Any(s => s.Spell != null || s.Action != WoWPetSpell.PetAction.None))
                     {
                         ...
                         {
                             if (sp.Spell == null)
                                 Logging.WriteDiagnostic("   {0} spell={1}  Action={0}", sp.ActionBarIndex, sp.ToString(), sp.Action.ToString());
                             else
                                 Logging.WriteDiagnostic("   {0} spell={1} #{2}", sp.ActionBarIndex, sp.ToString(), sp.Spell.Id);
                         }
