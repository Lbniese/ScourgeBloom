...
namespace ScourgeBloom.Class.DeathKnight
{
    public class Frost : ScourgeBloom
    {
        ...
        private static async Task<bool> CombatRoutine(WoWUnit onunit)
        {
            ...
            await Spell.CoCast(S.EmpowerRuneWeapon, Me, TTD.TimeToDeath(onunit) <= 60 && Me.CurrentTarget.IsBoss && Capabilities.IsCooldownUsageAllowed);

            //actions+=/plague_leech,if=disease.min_remains<1
            if (await Spell.CoCast(S.PlagueLeech, onunit,
                CanPlagueLeech() && DiseaseRemainsLessThanOne()))
                return true;
        ...
        private static async Task<bool> single_target_2h(WoWUnit onunit, bool reqs)
        {
            ...
            await single_target_bos(onunit, Me.HasAura(S.AuraBreathofSindragosa));
            ...
            if (await Spell.CoCast(S.Obliterate, onunit, BoSSelected() && Spell.GetCooldownLeft(S.BreathofSindragosa).TotalSeconds < 7 && Me.CurrentRunicPower < 76)) return true;
            ...
            if (await Spell.CoCast(S.HowlingBlast, onunit, BoSSelected() && Spell.GetCooldownLeft(S.BreathofSindragosa).TotalSeconds < 3 && Me.CurrentRunicPower < 88)) return true;
            //actions.single_target_2h+=/howling_blast,if=!talent.necrotic_plague.enabled&!dot.frost_fever.ticking
            if (await Spell.CoCast(S.HowlingBlast, onunit, !NecroticPlagueSelected() && !Me.CurrentTarget.HasMyAura(S.AuraFrostFever))) return true;
            //actions.single_target_2h+=/howling_blast,if=talent.necrotic_plague.enabled&!dot.necrotic_plague.ticking
            if (await Spell.CoCast(S.HowlingBlast, onunit, NecroticPlagueSelected() && !Me.CurrentTarget.HasMyAura(S.AuraNecroticPlague))) return true;
            //actions.single_target_2h+=/plague_strike,if=!talent.necrotic_plague.enabled&!dot.blood_plague.ticking
            if (await Spell.CoCast(S.PlagueStrike, onunit, !NecroticPlagueSelected() && !Me.CurrentTarget.HasMyAura(S.AuraBloodPlague))) return true;
            ...
            if (await Spell.CoCast(S.HowlingBlast, onunit, Me.HasAura("Rime") && DiseaseRemainsMoreThanFive())) return true;
        ...
        private static async Task<bool> multi_target(WoWUnit onunit, bool reqs)
        {
            ...
            await multi_target_bos(onunit, Me.HasAura(S.AuraBreathofSindragosa));
            ...
            if (await Spell.CoCast(S.PlagueStrike, onunit, Me.UnholyRuneCount == 2 && !Me.CurrentTarget.HasMyAura(S.AuraBloodPlague) && !NecroticPlagueSelected())) return true;
            ...
            if (await Spell.CoCast(S.FrostStrike, onunit, !BoSSelected() || BoSSelected() && Spell.GetCooldownLeft(S.BreathofSindragosa).TotalSeconds >= 10)) return true;
        ...
        private static async Task<bool> single_target_bos(WoWUnit onunit, bool reqs)
        {
            ...
            await Spell.CoCast(S.BloodTap, onunit,
                Me.HasAura("Killing Machine") && Me.HasAura(S.AuraBloodCharge) && Me.Auras["Blood Charge"].StackCount >= 5 &&
                SpellManager.CanCast(S.BloodTap));
            ...
            if (await Spell.CoCast(S.HowlingBlast, onunit, (Me.DeathRuneCount == 1 && Me.FrostRuneCount == 0 && Me.UnholyRuneCount == 0
            || Me.DeathRuneCount == 0 && Me.FrostRuneCount == 1 && Me.UnholyRuneCount == 0) && Me.CurrentRunicPower < 88)) return true;
...
#pragma warning restore 1998
        ...
        private static bool IsDualWielding => Me.Inventory.Equipped.MainHand != null && Me.Inventory.Equipped.OffHand != null;
        ...
        public override WoWClass Class => Me.Specialization == WoWSpec.DeathKnightFrost ? WoWClass.DeathKnight : WoWClass.None;
        ...
        private static async Task<bool> NecroBlightOpener(bool reqs)
        {
            ...
            if (
                await
                    Spell.CoCast(S.DeathsAdvance, TalentManager.IsSelected(7) && Capabilities.IsCooldownUsageAllowed))
                return true;
